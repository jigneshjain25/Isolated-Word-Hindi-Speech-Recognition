...
def mfcc(signal,samplerate=44100,winlen=0.043,winstep=0.01,numcep=13,
...
def fbank(signal,samplerate=44100,winlen=0.025,winstep=0.01,
    ...
    :returns: 2 values. The first is a numpy array of size (NUMFRAMES by nfilt) containing features. Each row holds 1 feature vector. The
        second return value is the energy in each frame (total energy, unwindowed). A numpy array of zero crossing rate for each frame.
    """          
...
def delta(m):
    ...
    for i in range(0,x):
        for j in range(0,y):
            delta_feat[i,j] = ( m[i,min(j+1,y-1)] - m[i,max(0,j-1)] + 2*(m[i,min(j+2,y-1)] - m[i,max(0,j-2)]) ) / 10.0
    return delta_feat
...
def zero_crossing_rate(frames):
	...
	:param frames: frames of signal for which zero_crossing_rate is calculated.
	...
	for i in range(0,Nframes):
		...
		N = len(frames[i])
		for j in range(1,N):
			zcr = zcr + abs(sgn(frames[i][j]) - sgn(frames[i][j-1]))
		zcr = zcr/(2.0*N)
		zcrs[i] = zcr
	return zcrs    
